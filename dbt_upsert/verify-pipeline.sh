#!/bin/bash
###############################################################################
# verify-pipeline.sh (dbt_upsert variant)
# Runs end-to-end diagnostics on the CDC pipeline with dbt merge layer.
# Usage: chmod +x verify-pipeline.sh && ./verify-pipeline.sh
# Run from the dbt_upsert/ directory (where docker-compose.yml lives).
###############################################################################
...
###############################################################################
header "STEP 6" "dbt — merge status"
###############################################################################

DBT_LOG=$(docker compose logs dbt 2>/dev/null | tail -50)
...
###############################################################################
header "STEP 8" "Live CDC test — insert and verify via dbt"
###############################################################################
...
# Trigger dbt run manually for faster verification
info "Triggering dbt run for immediate merge..."
docker compose exec -T dbt dbt run --project-dir /usr/app/dbt_project --profiles-dir /usr/app/dbt_project > /dev/null 2>&1
DBT_DONE=$(python3 -c "import time; print(int(time.time()*1000))")

# Now check output tables
info "Checking output tables after dbt run..."
FOUND=false
...
if [ "$LIVE_COUNT" = "1" ]; then
    ...
    DBT_ELAPSED=$(( DBT_DONE - STG_ARRIVED ))
    pass "Live CDC test passed — record merged into output_policy"
    info "Timing breakdown:"
    info "  Source -> Staging (Debezium+Kafka+Flink): ${STG_ELAPSED}ms"
    info "  Staging -> Output (dbt merge):            ${DBT_ELAPSED}ms"
...
if [ "$FOUND" = false ]; then
    fail "Live CDC test — record did not appear in output_policy after dbt run"
    info "Debug: docker compose exec dbt dbt run --project-dir /usr/app/dbt_project --profiles-dir /usr/app/dbt_project"
fi
...
if [ "$FOUND" = true ]; then
    # Get the policy_id for our test record
    ...
    # Run dbt to merge
    info "Triggering dbt run to merge update..."
    docker compose exec -T dbt dbt run --project-dir /usr/app/dbt_project --profiles-dir /usr/app/dbt_project > /dev/null 2>&1
    DBT_UPDATE_DONE=$(python3 -c "import time; print(int(time.time()*1000))")

    # Check that coverages and vehicles increased
    ...
    UPDATE_E2E=$(( DBT_UPDATE_DONE - UPDATE_START ))

    if [ "${COV_AFTER:-0}" -ge 2 ]; then
        pass "output_coverage: $COV_BEFORE -> $COV_AFTER (collision coverage added)"
    else
        fail "output_coverage: expected >= 2, got $COV_AFTER"
    fi

    if [ "${VEH_AFTER:-0}" -ge 2 ]; then
        pass "output_vehicle: $VEH_BEFORE -> $VEH_AFTER (Subaru Outback added)"
    else
        fail "output_vehicle: expected >= 2, got $VEH_AFTER"
    fi

    if [ "${DRV_AFTER:-0}" -ge 2 ]; then
        pass "output_driver: 1 -> $DRV_AFTER (Verify Partner added)"
    else
        fail "output_driver: expected >= 2, got $DRV_AFTER"
    fi

    if [ "$STG_UPDATE_FOUND" = true ]; then
        DBT_UPDATE_ELAPSED=$(( DBT_UPDATE_DONE - STG_UPDATE_ARRIVED ))
        info "Update timing breakdown:"
        info "  Source -> Staging (Debezium+Kafka+Flink): ${STG_UPDATE_ELAPSED}ms"
        info "  Staging -> Output (dbt merge):            ${DBT_UPDATE_ELAPSED}ms"
